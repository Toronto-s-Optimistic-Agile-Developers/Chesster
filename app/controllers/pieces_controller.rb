class PiecesController < ApplicationController
  ...
  def update
    ...
    if @piece.promotion? == true
      ...
      @game.reload
    elsif @piece.name == "Black_King"  && ((@piece.x_coord == 0) || (@piece.x_coord == 7)) && (@piece.y_coord == 0) && ! @piece.legal_move?(x_path, y_path)
      ...
      if @piece.black_right_castle(x_path, y_path) == true 
        unless King.legal_move?(x_path, y_path)
      ...
      if @piece.white_right_castle(x_path, y_path) == true 
        unless King.legal_move?(x_path, y_path)
          @piece.white_right_castle(x_path, y_path) unless valid_move?(x_path, y_path)
          flash[:notice] = 'You have successfully completed Castling.'
          @game.reload
        end
